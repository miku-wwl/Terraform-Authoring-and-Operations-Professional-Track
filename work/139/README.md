# 第 139 节做题环境：Launch Template 基础配置

这是你的上机做题目录。请编辑当前目录，不要修改 `practice/labs/139/` 中的参考实现。

本实验默认使用 Docker 启动 LocalStack 来模拟 AWS。不要使用真实 AWS 账号。

## 知识点总结

- Launch Template 保存 EC2 启动参数，后续可以给 Auto Scaling Group 复用。
- `image_id` 是 AMI，`instance_type` 是实例规格。
- `tag_specifications` 用来给通过模板创建出来的实例打标签。
...
```powershell
cd D:\workshop\GitHub\Terraform-Authoring-and-Operations-Professional-Track\work\139
