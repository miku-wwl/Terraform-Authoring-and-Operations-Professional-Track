...
这是你的上机做题目录。请编辑当前目录，不要修改 `practice/labs/103/` 中的参考实现。
...
```powershell
cd D:\workshop\GitHub\Terraform-Authoring-and-Operations-Professional-Track\work\103
...
- 新资源地址是 `aws_s3_bucket.instances["app"]` 和 `aws_s3_bucket.instances["logs"]`。
- `moved` block 分别表达 `aws_s3_bucket.a -> aws_s3_bucket.instances["app"]` 和 `aws_s3_bucket.b -> aws_s3_bucket.instances["logs"]`。
- `terraform plan` 不应出现非预期 destroy/create。
