...
本目录是 `work/49` 上机做题环境，来源于 `practice/49.md` 的实验设计。这里不是参考答案目录，你需要在当前目录内完成代码或命令练习。

核心主题：漂移、外部修改、`ignore_changes`

## 2. 任务目标

外部修改后 plan 不尝试恢复 content。

你需要根据题目目标修复起始文件中的 `TODO`，补全 ignore_changes 配置和忽略属性名称，让实验通过验收。
