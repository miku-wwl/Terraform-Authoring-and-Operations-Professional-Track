...
本目录是 `work/40` 上机做题环境，来源于 `practice/40.md` 的实验设计。这里不是参考答案目录，你需要在当前目录内完成代码或命令练习。

核心主题：`fileexists()`、多条件 precondition、依赖文件

## 2. 任务目标

前置文件存在时创建 `output/app.txt`。

你需要根据题目目标修复起始文件中的 `TODO`，补全 precondition 条件和输出表达式，让实验通过验收。
