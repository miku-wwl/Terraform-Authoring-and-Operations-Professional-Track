# Lab 79 network/backend.hcl
# 这个文件配置的是“网络团队这个 Terraform 项目自己的 state 存到哪里”。
...
# 这是上游 state。consumer/security 后面会读取这个 state 里的 outputs。
...
# Lab79 的重点是让下游安全团队读取网络团队公布的 outputs。
# 所以 network 先 apply，把 public_cidr 和 network_owner 写进自己的 remote state。
# 这里不启用 dynamodb_table，也不启用 use_lockfile，避免把锁机制混进本节主题。
...
# aws --endpoint-url=http://localhost:4566 s3 cp s3://tf-pro-state-localstack/labs/79/network/terraform.tfstate -
endpoints = {
