...
这是你的上机做题目录。请编辑 
etwork/ 和 consumer/，不要修改 practice/labs/79/ 中的参考实现。
...
完整实现 network 与 security 两个目录，security 通过 terraform_remote_state 自动读取 network 输出并生成规则说明。

## 要求

1. 在 
etwork/ 中创建一个输出 public_cidr 和 
etwork_owner 的配置，并把 state 写入 labs/79/network/terraform.tfstate。
