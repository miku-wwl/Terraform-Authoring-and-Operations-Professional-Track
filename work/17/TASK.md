...
本目录是 `work/17` 上机做题环境，来源于 `practice/17.md` 的实验设计。这里不是参考答案目录，你需要在当前目录内完成代码或命令练习。
...
你需要根据题目目标修复起始文件中的 `TODO`，补全 plan scan 命令，让实验通过验收。
...
`terraform test` 通过，并能生成 `plan.json` 供 Checkov plan scan 使用。
