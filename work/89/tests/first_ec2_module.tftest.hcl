run "first_ec2_module_is_created_and_called" {
  ...
  assert {
    condition = (
      output.team_a_instance_config.name == "team-a-web" &&
      output.team_a_instance_config.ami == "ami-0abcdef1234567890" &&
      output.team_a_instance_config.instance_type == "t2.micro" &&
      output.team_a_instance_config.region == "us-east-1" &&
      output.team_a_instance_config.managed_by == "terraform-module" &&
      output.team_a_instance_config.tags.Environment == "dev" &&
      output.team_a_instance_config.tags.Owner == "team-a" &&
      output.team_a_instance_config.tags.Module == "ec2"
    )
