...
本目录是 `work/23` 上机做题环境，来源于 `practice/23.md` 的实验设计。这里不是参考答案目录，你需要在当前目录内完成代码或命令练习。

核心主题：`-target`、上游依赖、下游资源、完整收敛

## 2. 任务目标

理解 target 的依赖方向，并最终完成完整收敛。

你需要根据题目目标修复起始文件中的 `TODO`，补全 target 与依赖方向的规则，让实验通过验收。
