...
[ "${AWS_ACCESS_KEY_ID:-}" = 'test' ] && [ "${AWS_SECRET_ACCESS_KEY:-}" = 'test' ] || {
  echo 'Lab 133 requires LocalStack test/test credentials.' >&2
  exit 1
}
...
case ",${services_line#SERVICES=}," in
  *,ec2,*) ;;
  *)
    echo 'LocalStack EC2 service is not enabled. Recreate the container with SERVICES=ec2.' >&2
    exit 1
    ;;
esac

echo 'PASS: LocalStack EC2 is healthy, endpoint is local, and credentials are test/test.'
