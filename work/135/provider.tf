...
variable "localstack_endpoint" {
  ...
  validation {
    ...
    error_message = "Lab 135 only permits http://localhost:4566 so it cannot accidentally manage real AWS IAM."
  }
}
...
provider "aws" {
  ...
  # 本 Lab 只创建 IAM objects；policy 中出现 S3/EC2 Action 不会调用对应服务 API。
  endpoints {
