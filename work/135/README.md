# 第 135 节做题环境：IAM Policy 与用户绑定

这是你的上机做题目录。请编辑当前目录，不要修改 `practice/labs/135/` 中的参考实现。

本实验默认使用 Docker 启动 LocalStack 来模拟 AWS。不要使用真实 AWS 账号。

## 知识点总结

- IAM policy 描述权限，attachment 描述策略绑定关系。
- `jsonencode` 能让你用 HCL 写策略结构，再由 Terraform 转成 JSON。
- 用户和策略通过 `aws_iam_user_policy_attachment` 连接。
...
```powershell
cd D:\workshop\GitHub\Terraform-Authoring-and-Operations-Professional-Track\work\135
