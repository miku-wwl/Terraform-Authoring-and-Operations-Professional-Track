...
这是你的上机做题目录。请编辑当前目录，不要修改 `practice/labs/101/` 中的参考实现。
...
```powershell
cd D:\workshop\GitHub\Terraform-Authoring-and-Operations-Professional-Track\work\101
...
- 新资源地址是 `aws_s3_bucket.renamed`。
- `moved` block 表达从 `aws_s3_bucket.original` 到 `aws_s3_bucket.renamed` 的 state 地址迁移。
