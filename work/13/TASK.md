...
本目录是 `work/13` 上机做题环境，来源于 `practice/13.md` 的实验设计。这里不是参考答案目录，你需要在当前目录内完成代码或命令练习。
...
你需要根据题目目标修复起始文件中的 `TODO`，补全安全发现列表，让实验通过验收。
...
`terraform test` 返回 `1 passed, 0 failed`，Checkov 扫描命令返回结果。
