...
> ⚠️ **SKIP** — 本实验涉及真实云资源 import（安全组、EC2 等），不适合本地模拟。
> 计划后续在 **Azure 云环境** 中实操。当前暂不做。

## 1. 背景

本目录是 `work/19` 上机做题环境，来源于 `practice/19.md` 的实验设计。这里不是参考答案目录，你需要在当前目录内完成代码或命令练习。

核心主题：Terraform import、import block、生成配置、状态接管

## 2. 任务目标

实验能输出 import 的关键步骤，并保留被接管资源的业务 ID。

你需要根据题目目标修复起始文件中的 `TODO`，补全 import 工作流命令和遗留资源 ID，让实验通过验收。
