...
本目录是 `work/27` 上机做题环境，来源于 `practice/27.md` 的实验设计。这里不是参考答案目录，你需要在当前目录内完成代码或命令练习。

核心主题：`<<EOT`、`<<-EOT`、前导空白

## 2. 任务目标

基础 heredoc 保留外层缩进，indented heredoc 去掉共同缩进。

你需要根据题目目标修复起始文件中的 `TODO`，将 indented heredoc 改为 `<<-EOT`，让实验通过验收。
