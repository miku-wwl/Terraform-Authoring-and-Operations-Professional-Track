...
2. 遍历 CSV 数据，构造 `processed_rules` list。
3. 在 `processed_rules` 中保留 `name`、`direction`、`protocol`、`cidr_block` 字段。
4. 用 `can(regex("-", rule.port))` 判断端口是否为范围。
5. 用 `split("-", rule.port)` 和 `tonumber()` 生成 `from_port` / `to_port`。
6. 用 `for index, rule in local.processed_rules` 构造 `ingress_rules_by_key`，确保每个 `for_each` key 唯一。
